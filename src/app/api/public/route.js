...
// No importamos ninguna dependencia de email
...
async function triggerDirectEmailNotification(responseId) {
  try {
    ...
    // ¡Usamos el nuevo endpoint directo!
    const notifyUrl = `${baseUrl}/api/direct-notify?response_id=${responseId}`;
    
    console.log('Triggering direct email notification via separate HTTP call:', notifyUrl);
    ...
    if (response.ok) {
      console.log('Direct email notification triggered successfully:', result);
      return { success: true };
    } else {
      console.error('Failed to trigger direct email notification:', result);
      return { success: false, error: result.error };
    }
  } catch (error) {
    console.error('Error triggering direct email notification:', error);
...
export async function POST(request) {
  ...
  try {
    ...
    // AUTOMATIZACIÓN: Usamos el nuevo endpoint que usa la API directa de SendGrid
    console.log('Attempting to trigger direct email notification for response ID:', response_id);
    triggerDirectEmailNotification(response_id).catch(error => {
      console.error('Failed to trigger direct email notification:', error);
    });
